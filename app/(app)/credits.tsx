...
export default function CreditsScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F1F1F3" }}>
      ...
      >
        <HookPageHeader title="Hook Credits" subtitle="Spend credits on your next order." />
        ...
        <View style={{ marginTop: 24, borderRadius: 22, backgroundColor: "#111111", padding: 20 }}>
          ...
          <Text style={{ marginTop: 8, fontSize: 13, lineHeight: 21, color: "#DDDDDD" }}>
            Credits can cover up to {credits.data?.capPercent ?? 20}% of each order, so a bigger basket unlocks more
            of your balance.
          </Text>
        </View>
        ...
        {credits.isLoading ? (
          ...
          <Pressable accessibilityRole="button" onPress={() => void refresh()} style={{ padding: 24, marginTop: 20, borderRadius: 22, backgroundColor: "white" }}>
            <Text style={{ color: "#111", textAlign: "center" }}>Couldn’t load credits. Tap to retry.</Text>
          </Pressable>
        ) : !history.length ? (
          <View style={{ marginTop: 20, alignItems: "center", borderRadius: 22, backgroundColor: "white", padding: 24, gap: 12 }}>
            ...
            <Text className="mt-3 text-base font-bold text-black">No credits yet</Text>
            <Text className="mt-1 text-center text-sm leading-5 text-[#666]">
              Invite a friend to Hook and you both earn credits on their first order.
            </Text>
            <Pressable
